    ...
    <section className="hairline bg-[color:var(--color-surface-container-lowest)] p-6 sm:p-10">
      <div className="flex flex-col gap-2 mb-6 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-[14px] italic text-[color:var(--color-on-surface-variant)]">
            Click any day for hormone state, cognitive notes, and recommendations.
          </p>
        </div>
        <Legend />
      </div>
      ...
      <div className="grid grid-cols-7 gap-1.5">
        {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((d) => (
          <div
            key={d}
            className="text-center text-[10px] font-semibold uppercase tracking-[0.16em] text-[color:var(--color-on-surface-variant)]"
          >
            {d}
          </div>
        ))}
...
function DayCell({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`relative flex aspect-square flex-col items-center justify-center transition-all duration-200 ${phaseLight[day.phase]} ${
    ...
    >
      ...
      <span className="mt-0.5 text-[8px] font-semibold uppercase tracking-[0.12em] opacity-60">
        {profile.shortLabel}
      </span>
    </button>
  );
